...
public class ConeR3DualBalance extends SequentialCommandGroup {

    public ConeR3DualBalance (SwerveAutoBuilder autoBuilder, PoseEstimator s_poseEstimator) {

        PathPlannerTrajectory ConeR3DualBalance = PathPlanner.loadPath("ConeR3DualBalance", new PathConstraints(2, 1));
        Pose2d startpose = ConeR3DualBalance.getInitialHolonomicPose();

        addCommands(new SequentialCommandGroup(
            new InstantCommand(()-> s_poseEstimator.resetPose(startpose)),
            autoBuilder.fullAuto(ConeR3DualBalance)
        ));
    }
}